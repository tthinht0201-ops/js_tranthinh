import { adminAppointmentsQuerySchema, adminUpdateAppointmentStatusSchema, appointmentIdSchema, cancelAppointmentSchema, createAppointmentSchema, doctorAppointmentsQuerySchema, patientAppointmentsQuerySchema, adminRescheduleAppointmentSchema, } from "./appointment.schema.js";
import { cancelMyPatientAppointment, completeMyDoctorAppointment, createAppointment, getAllAppointments, getMyDoctorAppointments, getMyPatientAppointments, updateAppointmentStatusByStaff, rescheduleAppointmentByStaff, } from "./appointment.service.js";
...
const handleAppointmentError = (error, res) => {
...
export const getMyPatientAppointmentsController = async (req, res) => {
...
export const cancelMyPatientAppointmentController = async (req, res) => {
...
export const getMyDoctorAppointmentsController = async (req, res) => {
...
export const completeMyDoctorAppointmentController = async (req, res) => {
...
export const updateAppointmentStatusByStaffController = async (req, res) => {
    const params = appointmentIdSchema.safeParse(req.params);
...
export const rescheduleAppointmentByStaffController = async (req, res) => {
    const params = appointmentIdSchema.safeParse(req.params);
    const body = adminRescheduleAppointmentSchema.safeParse(req.body);
    if (!params.success || !body.success) {
        res.status(400).json({
            success: false,
            message: "Dữ liệu đổi lịch không hợp lệ",
            errors: body.success
                ? undefined
                : body.error.flatten(),
        });
    ...
    try {
        const data = await rescheduleAppointmentByStaff(params.data.id, body.data);
        res.status(200).json({
            success: true,
