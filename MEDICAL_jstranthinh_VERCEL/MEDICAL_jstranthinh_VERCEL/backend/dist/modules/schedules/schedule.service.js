...
import { dateAtVietnamMinute, formatVietnamTime, getDayOfWeekFromDateString, getVietnamMinuteOfDay, parseVietnamDateEnd, parseVietnamDateStart, toVietnamDateString, } from "../../utils/time.js";
const DOCTOR_BLOCK_MESSAGE = "Bác sĩ có việc đột xuất. Phòng khám sẽ liên hệ để hỗ trợ đổi lịch hoặc hủy lịch.";
...
const parseWorkDate = (date) => {
    return new Date(`${date}T00:00:00.000Z`);
};
const workDateToString = (date) => {
...
export const getMySchedules = async (userId) => {
...
export const createMySchedule = async (userId, input) => {
    const doctor = await getDoctorByUserId(userId);
...
export const updateMySchedule = async (userId, scheduleId, input) => {
    const doctor = await getDoctorByUserId(userId);
...
export const deleteMySchedule = async (userId, scheduleId) => {
    const doctor = await getDoctorByUserId(userId);
    const schedule = await prisma.doctorSchedule.findFirst({
        where: {
            id: scheduleId,
...
const ensureNoDateScheduleOverlap = async (tx, params) => {
...
const getEffectiveSchedulesForDate = async (tx, doctorId, date) => {
...
const appointmentFitsSchedules = (appointment, schedules) => {
    const startMinute = getVietnamMinuteOfDay(appointment.startAt);
    const durationMinutes = Math.round((appointment.endAt.getTime() - appointment.startAt.getTime()) / 60_000);
    return schedules.some((schedule) => startMinute >= schedule.startMinute &&
...
const reconcileAppointmentsForDateSchedule = async (tx, doctorId, date) => {
...
export const getMyDateSchedules = async (userId) => {
    const doctor = await getDoctorByUserId(userId);
...
export const createMyDateSchedule = async (userId, input) => {
    const doctor = await getDoctorByUserId(userId);
    validateScheduleRange(input);
...
export const updateMyDateSchedule = async (userId, scheduleId, input) => {
    const doctor = await getDoctorByUserId(userId);
    return prisma.$transaction(async (tx) => {
...
export const deleteMyDateSchedule = async (userId, scheduleId) => {
    const doctor = await getDoctorByUserId(userId);
    return prisma.$transaction(async (tx) => {
        const schedule = await tx.doctorDateSchedule.findFirst({
            where: {
...
const markConflictingAppointments = async (tx, params) => {
    const appointments = await tx.appointment.findMany({
        where: {
            doctorId: params.doctorId,
            status: {
...
export const getMyBlockedTimes = async (userId, query) => {
    const doctor = await getDoctorByUserId(userId);
...
export const createMyBlockedTime = async (userId, input) => {
    const doctor = await getDoctorByUserId(userId);
...
export const updateMyBlockedTime = async (userId, blockedTimeId, input) => {
    const doctor = await getDoctorByUserId(userId);
    ...
    return prisma.$transaction(async (tx) => {
        ...
        for (const appointment of previouslyAffected) {
            const stillOverlaps = appointment.startAt < endAt && appointment.endAt > startAt;
            if (!stillOverlaps) {
                const previousStatus = appointment.reschedulePreviousStatus === AppointmentStatus.CONFIRMED
                    ? AppointmentStatus.CONFIRMED
                    : AppointmentStatus.PENDING;
                await tx.appointment.update({
                    where: {
        ...
        const datesToReconcile = new Set(previouslyAffected.map((appointment) => toVietnamDateString(appointment.startAt)));
...
export const deleteMyBlockedTime = async (userId, blockedTimeId) => {
    const doctor = await getDoctorByUserId(userId);
    const blockedTime = await prisma.blockedTime.findFirst({
        where: {
            id: blockedTimeId,
    ...
    return prisma.$transaction(async (tx) => {
        ...
        const datesToReconcile = new Set(affectedAppointments.map((appointment) => toVietnamDateString(appointment.startAt)));
        for (const date of datesToReconcile) {
            await reconcileAppointmentsForDateSchedule(tx, doctor.id, date);
        }
        return {
...
export const getAvailableSlots = async (doctorId, date) => {
    ...
    const workDate = parseWorkDate(date);
    const dateSchedules = await prisma.doctorDateSchedule.findMany({
        where: {
            doctorId,
            workDate,
            isActive: true,
    ...
    const schedules = dateSchedules.length > 0 ? dateSchedules : weeklySchedules;
    const now = new Date();
    const slots = [];
    for (const schedule of schedules) {
        for (let minute = schedule.startMinute; minute + schedule.slotMinutes <= schedule.endMinute; minute += schedule.slotMinutes) {
            ...
            const isBlocked = blockedTimes.some((blocked) => blocked.startAt < endAt && blocked.endAt > startAt);
            if (isBlocked) {
                continue;
            }
            const isBooked = appointments.some((appointment) => appointment.startAt < endAt && appointment.endAt > startAt);
            if (isBooked) {
                continue;
            }
            slots.push({
