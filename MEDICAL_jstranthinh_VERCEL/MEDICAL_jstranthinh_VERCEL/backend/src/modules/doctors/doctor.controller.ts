...
const handleDoctorServiceError = (
  error: unknown,
  res: Response,
): boolean => {
...
export const getDoctorsController = async (
  ...
  const parsed =
    doctorQuerySchema.safeParse(req.query);
...
export const getDoctorsForStaffController = async (
  _req: Request,
  res: Response,
): Promise<void> => {
...
export const getDoctorByIdController =
  async (
    ...
    const parsed =
      doctorIdSchema.safeParse(req.params);
...
export const deactivateDoctorController =
  async (
    req: Request,
    res: Response,
  ): Promise<void> => {
    ...
    try {
      ...
      res.status(200).json({
        success: true,
        message:
          "Ngừng hoạt động bác sĩ thành công",
        data: doctor,
      });
    } catch (error) {
      if (
