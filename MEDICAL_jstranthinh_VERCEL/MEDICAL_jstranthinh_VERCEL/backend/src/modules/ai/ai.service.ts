...
const DISCLAIMER =
  "Gợi ý này chỉ nhằm hỗ trợ lựa chọn chuyên khoa, không phải chẩn đoán y khoa. Nếu triệu chứng nặng, diễn tiến nhanh hoặc có dấu hiệu cấp cứu, hãy liên hệ cơ sở y tế phù hợp.";

const keywordGroups: Array<{ keywords: string[]; specialtyHints: string[] }> = [
  {
...
const findHeuristicSpecialty = (
  symptoms: string,
  specialties: Array<{ id: string; name: string; description: string | null }>,
) => {
...
export const suggestSpecialty = async (input: SpecialtySuggestionInput) => {
